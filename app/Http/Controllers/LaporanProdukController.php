...
class LaporanProdukController extends Controller
{
    ...
    public function laporan_produk(Request $request)
    {
        $data_laporan = DB::table('mengelola_produk')
            ...
            $listproduk = DB::table('produk')->select('id_produk')->get();
    ...
    public function rekapproduk()
    {
        ...
        if (request()->has('tanggal_awal') && request('tanggal_awal') !== '') {
            ...
            $data_laporan->whereRaw('DATE(mengelola_produk.created_at) >= ?', [date('Y-m-01')]);
        }

        if (request()->has('tanggal_akhir') && request('tanggal_akhir') !== '') {
            $data_laporan->whereDate('mengelola_produk.created_at', '<=', request('tanggal_akhir'));
        } else {
            $data_laporan->whereRaw('DATE(mengelola_produk.created_at) <= ?', [date('Y-m-t')]);
        }
...
public function produk_Excel(Request $request)
{
     $data_laporan = DB::table('mengelola_produk')
        ...
        ->leftJoin('produk', 'mengelola_produk.id_produk', '=', 'produk.id_produk')
        ->when($request->filled('tanggal_awal'), fn($q) => $q->whereDate('mengelola_produk.created_at', '>=', $request->tanggal_awal))
        ->when($request->filled('tanggal_akhir'), fn($q) => $q->whereDate('mengelola_produk.created_at', '<=', $request->tanggal_akhir))
        ->when($request->filled('id_produk') && $request->id_produk !== 'semua', fn($q) => $q->where('mengelola_produk.id_produk', $request->id_produk))
        ->when($request->filled('jenis_pencatatan') && $request->jenis_pencatatan !== 'semua', fn($q) => $q->where('mengelola_produk.jenis_pencatatan', $request->jenis_pencatatan))
        ->orderBy('tanggal', 'asc')
        ->get();
    ...
    $sheet->setCellValue('B1', 'ID Kelola');
