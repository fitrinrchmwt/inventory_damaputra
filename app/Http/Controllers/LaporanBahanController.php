    ...
    public function rekapbahan()
    {
        ...
        ->leftJoin('bahanbaku', 'mengelola_bahan.id_bahan', '=', 'bahanbaku.id_bahan');
         ...
         if (request()->has('tanggal_awal') && request('tanggal_awal') !== '') {
            ...
            $data_laporanbahan->whereRaw('DATE(mengelola_bahan.created_at) >= ?', [date('Y-m-01')]);
        }

        if (request()->has('tanggal_akhir') && request('tanggal_akhir') !== '') {
            $data_laporanbahan->whereDate('mengelola_bahan.created_at', '<=', request('tanggal_akhir'));
        } else {
            $data_laporanbahan->whereRaw('DATE(mengelola_bahan.created_at) <= ?', [date('Y-m-t')]);
        }
    ...
    public function bahan_Excel(Request $request)
{
     $data_laporanbahan = DB::table('mengelola_bahan')
        ...
        ->leftJoin('bahanbaku', 'mengelola_bahan.id_bahan', '=', 'bahanbaku.id_bahan')
        ->when($request->filled('tanggal_awal'), fn($q) => $q->whereDate('mengelola_bahan.created_at', '>=', $request->tanggal_awal))
        ->when($request->filled('tanggal_akhir'), fn($q) => $q->whereDate('mengelola_bahan.created_at', '<=', $request->tanggal_akhir))
        ->when($request->filled('id_bahan') && $request->id_bahan !== 'semua', fn($q) => $q->where('mengelola_bahan.id_bahan', $request->id_bahan))
        ->when($request->filled('jenis_pencatatan') && $request->jenis_pencatatan !== 'semua', fn($q) => $q->where('mengelola_bahan.jenis_pencatatan', $request->jenis_pencatatan))
        ->orderBy('tanggal', 'asc')
        ->get();
